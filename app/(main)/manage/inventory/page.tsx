...
export default async function InventoryPage() {
    ...
    const { data: categories, error } = await supabase.from('categories').select(`category_id, name, photo_url, subcategories(subcategory_id, name)`).order("name", { ascending: true }).order("name", { foreignTable: "subcategories", ascending: true })
    if (error) {
        console.error(error)
        return <div>Error loading inventory items</div>
    }

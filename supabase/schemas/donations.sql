...
on public.donations
for select to anon -- applies to anon role (anonymous/public users)
using (true); -- all rows satisfy condition, all rows are readable

-- insert
create policy "public can insert entries in donations"
on public.donations
for insert to anon --  applies to anon role (anonymous/public users)
with check (true); -- for insert/update row validation, only rows where condition evaluates to true can be inserted/updated
